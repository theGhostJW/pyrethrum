...
filterredItemIds :: forall i ds. (ItemClass i ds) => ItemFilter i -> [i] -> Either ItemFilterError (S.Set Int)
filterredItemIds filtr items =
  let
    filterredItems :: Either ItemFilterError [Int]
    filterredItems = let
    ...
    checkIds :: Either ItemFilterError ()
    checkIds =
      let
