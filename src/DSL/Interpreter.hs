...
type FileSystemEffs a = forall effs. (Members '[Ensure, FileSystem] effs) => (Eff effs a)

type InteractorFileSystem runConfig item r = forall v. (TestItem item v) => runConfig -> item -> FileSystemEffs r
