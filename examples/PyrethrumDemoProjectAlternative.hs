module PyrethrumDemoProjectAlternative where
...
-- onceIntChildWrong :: OnceBefore' ThreadBefore Int Int
-- onceIntChildWrong = OnceBefore' threadBeforeInt $ \_ i -> pure $ i + 1

data OnceBefore i o where
  OnceBefore ::
    { action :: RunConfig -> Action o
    } ->
    OnceBefore () o

data OnceAfter o where
  OnceAfter ::
    { action :: RunConfig -> Action ()
    } ->
    OnceAfter o

data OnceAround o where
  OnceAround ::
...
data ThreadBefore o where
  ThreadBefore ::
    { action :: RunConfig -> Action o
    } ->
    ThreadBefore o

data ThreadAfter o where
  ThreadAfter ::
    { action :: RunConfig -> Action ()
    } ->
    ThreadAfter o

data ThreadAround o where
  ThreadAround ::
    { setup :: RunConfig -> Action o
    , teardown :: RunConfig -> o -> Action ()
    } ->
...
data OnceBefore' i o where
  OnceBefore' ::
    (Hook hook, CanDepend OnceBefore' hook) =>
    { depends :: hook i' i
    , action :: RunConfig -> i -> Action o
    } ->
    OnceBefore' i o

instance Hook OnceBefore 
instance Hook OnceBefore'

instance CanDepend OnceBefore' OnceBefore
instance CanDepend OnceBefore' OnceBefore'

onceBeforeTxt :: OnceBefore () Text
onceBeforeTxt = OnceBefore $ \_ -> pure "1"


onceBefore :: OnceBefore () Integer
onceBefore = OnceBefore $ \_ -> pure 1

onceAround :: OnceAround Integer
onceAround = OnceAround (\_ -> pure 1) (\_ _ -> pure ())


-- onceBefore' :: OnceBefore' Integer Integer
onceBefore' :: OnceBefore' Integer Integer
onceBefore' =
  OnceBefore'
    { depends = onceBefore
...
onceBefore2' :: OnceBefore' Integer Text
onceBefore2' =
  OnceBefore'
    { depends = onceBefore
    , action = \_ _i -> pure "Hello"
    }
...
onceBefore2'' :: OnceBefore' Text Int
onceBefore2'' =
  OnceBefore'
    { depends = onceBefore2'
    , action = \_ _i -> pure 1
...
-- onceBeforeWrong2' :: OnceBefore' Integer Integer
-- onceBeforeWrong2' = OnceBefore' onceBeforeTxt $ \_ i -> pure $ i + 1

threadBefore :: ThreadBefore Integer
threadBefore = ThreadBefore $ \_ -> pure 1

-- onceBefore'Wrong :: OnceBefore' Integer Integer
-- onceBefore'Wrong  = OnceBefore' threadBefore $ \_ i -> pure $ i + 1

-- TODO ::
-- add other constructors
-- add convertable typeclasses and reimplemnt conversion
-- move instance to own testClass
-- convert example
-- manually implement create suite

-- data ThreadHook o where
--   ThreadBefore2 ::
--     {
