...
-- $> unit_failure
unit_failure :: IO ()
unit_failure = replicateM_ 1000 intermittentFailure

intermittentFailure :: IO ()
intermittentFailure =
    runTest'
            ...
            , subNodes =
                [ OnceBefore
                    ...
                    , subNodes =
                        [ Fixture
                            { tests =
                                [ Spec{delay = 24, result = Pass}
                                , Spec{delay = 0, result = Pass}
                                , Spec{delay = 18, result = Pass}
                                , Spec{delay = 20, result = Fail}
                                , Spec{delay = 3, result = Pass}
                                ]
                            }
                        , OnceBefore
                            { spec = Spec{delay = 0, result = Pass}
        ...
        , Fixture
            { tests =
                [ Spec{delay = 16, result = Pass}
                , Spec{delay = 134, result = Fail}
                , Spec{delay = 0, result = Pass}
                , Spec{delay = 320, result = Pass}
                , Spec{delay = 12, result = Pass}
                , Spec{delay = 4, result = Pass}
                ]
            }
        ]