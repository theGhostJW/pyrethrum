...
import Prelude (Bool (..), Exception, IO, Integer, Maybe (..), Show, Text, ($), (.), (==), (||))
...
import PyrethrumExtras (MonadMask, toS, uu)
...
data FSException = FSException
  deriving (Show)

instance Exception FSException

runFileSystem :: (HasCallStack, IOE :> es, E.Error FSException :> es) => Eff (FileSystem : es) a -> Eff es a
runFileSystem =
  interpret $ \_ -> \case
    EnsureDir p -> uu
...
-}
ensureDir :: (HasCallStack, FileSystem :> es, IOE :> es) => Path b Dir -> Eff es ()
ensureDir = R.ensureDir

{-
createDir :: (FileSystem :> es) => Path b Dir -> Eff es ()
createDir = unsafeEff_ . R.createDir

-- | Lifted 'R.createDirIfMissing'.
